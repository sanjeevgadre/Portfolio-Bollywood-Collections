...
#%% Exhibitor Share v/s Year
X = movie_master['release_year']
Y = ex_share
corr = X.corr(Y, method = 'spearman')
print('Total Effect of Y on F : %.4f' % corr)

#%% Exhibitor v/s Year, conditioned on Inflation
X = movie_master['release_year']
...
X = movie_master.loc[:, ['budget', 'india-footfalls']]
X = pd.concat([X, fwr, run_length], axis = 1)
X.columns = ['budget', 'india-footfalls', 'fwr', 'runlen']
Y = ex_share
Y[1094] = 1             # hack

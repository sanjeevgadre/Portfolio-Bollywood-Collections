...
#%% Run Length v/s First Week Revenue conditioned on Budget, Screens, Inflation and Year
X = movie_master.loc[:, ['budget', 'screens', 'inf_adj_fct', 'release_year']]
X = pd.concat([X, fwr], axis = 1)
X.columns = ['budget', 'inf_adj_fct', 'release_year', 'screens', 'fwr']
X = X.astype('float')
Y = run_len
X = (X - X.mean())/X.std()
Y = (Y - Y.mean())/Y.std()
...
#%% Run Length v/s Budget, conditioned on Release Year, Screens, Runtime and First Week Revenue
...
#%% Run Length v/s Year, conditioned on Screens, Budget and First Week Revenue
